...
public static class MeshGenerator
{
	public static MeshData GenerateTerrainMesh(float[,] heightMap, float heightMultiplier, AnimationCurve _heightCurve, int levelOfDetail)
	{
		...
		{
			for (int x = 0; x < borderedSize; x += meshSimplificationIncrement, vertexIndex++)
			{
				bool isBorderVertex = y == 0 || y == borderedSize - 1 || x == 0 || x == borderedSize - 1;

				if (isBorderVertex == true)
				{
...
public class MeshData
{
	...
	public void AddVertex(Vector3 vertexPosition, Vector2 uv, int index)
	{
		if (index < 0)
		{
			borderVertices[-index - 1] = vertexPosition;
		}
		else
		{
	...
	Vector3[] CalculateNormals()
	{
		...
		{
			int normalTriangleIndex = i * 3;
			int indexA = borderTriangles[normalTriangleIndex];
			int indexB = borderTriangles[normalTriangleIndex + 1];
			int indexC = borderTriangles[normalTriangleIndex + 2];
	...
	{
		Vector3 pointA = indexA >= 0 ? vertices[indexA] : borderVertices[-indexA - 1];
		Vector3 pointB = indexB >= 0 ? vertices[indexB] : borderVertices[-indexB - 1];
		Vector3 pointC = indexC >= 0 ? vertices[indexC] : borderVertices[-indexC - 1];
